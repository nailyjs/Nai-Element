...
(async function bootstrap() {
  ...
  const name = configService.getOrThrow("gpt.name");

  // Swagger
  await SwaggerModule.loadPluginMetadata(metadata);
  const [openAPIObject, generate] = EnableSwagger(app, "gpt", (builder) => {
    return builder.setTitle(name).setDescription(readFileSync(join(process.env.PROJECT_ROOT, "apps/shop/DESC.md")).toString("utf-8"));
  });
  generate("gpt.openapi.json", openAPIObject).then();
  await app.listen(port);
  return app;
})().then(async (app) => {
  new CommonLogger("NestApplication").verbose(`GPT app is running on ${await app.getUrl()}`);
});
